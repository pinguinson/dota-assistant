...
object DotaApiConfig {
  ...
  case class DotaApiConfig(endpoints: Endpoints, validLobbyTypes: Seq[Int], maxRecentGames: Int, apiKey: String)
  private case class DotaApi(dotaApi: DotaApiConfig)

  private val defaultConfig = DotaApiConfig(Endpoints("missing", "missing"), List.empty, 1, "missing")
